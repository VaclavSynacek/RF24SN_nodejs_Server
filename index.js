...
var RawPacket = _.struct([
    ...
    _.uint8("nodeId"),
    _.uint8("reserved"),
    _.uint8("packetType")]   // client node id (unique within one RF24Star network)
)
...
radio.channel(0x4c).dataRate('1Mbps').crcBytes(2).autoRetransmit({count:15, delay:500})

var listeningPipe;
var replyPipes = new Array();


//nRF24l01 listening handlers
radio.begin(function () {
    var listeningPipe = radio.openPipe('rx', 0xF0F0F0F000, {size: RawPacket.size, autoAck:false});
...
var sendPacket = function(packet) {
	...
	console.log("adding pipe for node " + packet.nodeId);
        replyPipes[packet.nodeId] = radio.openPipe('tx', 0xF0F0F0F000+packet.nodeId, {size: RawPacket.size, autoAck:false});
